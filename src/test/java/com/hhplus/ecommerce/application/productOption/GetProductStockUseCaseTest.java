...
@DisplayName("GetProductStockUseCase 단위 테스트")
class GetProductStockUseCaseTest {
    ...
    void setUp() {
        ...
        testOptions = Arrays.asList(
            ...
            new ProductOptionEntity(3L, 1L, "64GB RAM", 400000L, 10L, timestamp, timestamp)
        );
    }

    @Test
    @DisplayName("유효한 상품 ID로 재고 정보를 조회한다")
    void execute_ValidProductId_ReturnsStockResponse() {
        // given
        Long productId = 1L;
    ...
    @Test
    @DisplayName("옵션이 없는 상품의 재고를 조회한다")
    void execute_ProductWithNoOptions_ReturnsEmptyOptions() {
        // given
        Long productId = 1L;
        given(productRepository.findById(productId)).willReturn(Optional.of(testProduct));
    ...
    void execute_NullProductId_ThrowsException() {
        ...
        // when & then
        assertThatThrownBy(() -> getProductStockUseCase.execute(nullId))
    ...
    void execute_InvalidProductId_ThrowsException() {
        ...
        // when & then
        assertThatThrownBy(() -> getProductStockUseCase.execute(invalidId))
            .isInstanceOf(InvalidInputException.class)
